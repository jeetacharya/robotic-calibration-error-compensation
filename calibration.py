...
bin_edges = [x * 0.005 for x in range(11)]                        # define bin boundaries for plotting histogram of errors
...
ee_pre_cal_error = np.sqrt(((x_measured_pre_cal - x_predicted) ** 2) + ((y_measured_pre_cal - y_predicted) ** 2))       # pre-calibration error of end-effector position
ee_post_cal_error = np.sqrt(((x_measured_post_cal - x_predicted) ** 2) + ((y_measured_post_cal - y_predicted) ** 2))    # post-calibration error of end-effector position
ee_pre_cal_error_rms = np.sqrt(np.mean(ee_pre_cal_error ** 2))                     # pre-calibration rms error of end-effector position
ee_post_cal_error_rms = np.sqrt(np.mean(ee_post_cal_error ** 2))                   # post-calibration rms error of end-effector position
ee_pre_cal_error_rms = np.round(ee_pre_cal_error_rms, decimals = 3)
ee_post_cal_error_rms = np.round(ee_post_cal_error_rms, decimals = 3)


# Generating scatter plot of end-effector positions and error histogram
...
ax2.hist(ee_post_cal_error, bins = bin_edges, color = 'skyblue', edgecolor = 'black')
...
print(f"Pre-calibration RMS error for end-effector position = {ee_pre_cal_error_rms} m")
print(f"Post-calibration RMS error for end-effector position = {ee_post_cal_error_rms} m")
...
get_ipython().system('jupyter nbconvert --to script calibration.ipynb')
